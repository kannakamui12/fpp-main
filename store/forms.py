...
from store.models import Address, contact
...
class AddressForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {'locality': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Cth: JL. M.Yamin, Gn. Kalua no.25'}), 'city': forms.TextInput(
            attrs={'class': 'form-control', 'placeholder': 'Cth: Samarinda Kota'}), 'state': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Cth: Gedung Ungu, Diseberang Dealer Honda'})}
...
class contactform(forms.ModelForm):
    class Meta:
        model = contact
        fields = ('name','address','phone','payment')
        labels = {
            'name' : '',
            'address' : '',
            'phone' : '',
